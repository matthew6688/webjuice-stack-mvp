...
if (!caseDir && (!clientSlug || !orderId)) {
  console.error('Usage: node scripts/ops/send-review-email.js --client <slug> --order <runId> [--send true]');
  console.error('   or: node scripts/ops/send-review-email.js --case-dir data/cases/<client>/<runId> [--send true]');
  process.exit(1);
}

const result = await sendReadyForReviewEmail({
  clientSlug,
  orderId,
  caseDir,
  send,
