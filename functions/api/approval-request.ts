...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    const request = buildApprovalWorkflowDispatch(body, context.env);
    if (!request.ok) {
    ...
    if (webhookUrl) context.waitUntil(sendJson(webhookUrl, buildDiscordPayload(request.discordFields)));
    context.waitUntil(dispatchWorkflow(context.env, request.workflow, request.inputs, {
      provider: 'approval',
      fields: { ...request.inputs, repo: request.discordFields.repo },
    }));

    return json({ success: true, clientSlug: request.inputs.client_slug, repo: request.discordFields.repo });
  } catch (error) {
