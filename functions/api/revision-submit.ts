...
async function sendRevisionEmail(env: Env, payload: Record<string, string | string[]>, files: Array<{ filename: string; content: string; content_type: string; size: number }>) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      subject: `New revision request: ${payload.business_name || payload.client_slug || payload.order_id}`,
      text,
      html: renderProfitsLocalEmail({
        eyebrow: 'Revision request',
        subject: `New revision request: ${payload.business_name || payload.client_slug || payload.order_id}`,
        intro: 'A customer submitted revision notes or assets for an active ProfitsLocal project.',
        details,
        closing: 'Check the order match and revision allowance before assigning build work.',
