...
- [SOP-READY-TO-BUILD.md](./SOP-READY-TO-BUILD.md) · M2 → M3 qualification gate (D39 草稿 · Hard gates + 5 维 scorecard · 等 Matthew review)
...
| 项 | 工作量 | 价值 |
|---|---|---|
| **M1 / M2 流程验证** (Matthew 当前正在做) | 进行中 | confirm 4 入口 + audit 链路全通 |
| **Ready-to-Build qualification gate 实装 (D39)** · 见 SOP-READY-TO-BUILD.md · 5 维 scorecard + 7 hard gates + multi-page crawl + AI 分析 raw JSON | ~13h · $1-2/customer | 现在我们对每个 A/B/C 都 auto build demo · 错!  · 真实是 A/B 后还有筛选才到 build. M2→M3 之间的 gate |
| **master.md → demo 管道优化** (M3 reference-adapter 输出质量) · 等 D39 + 渠道可视化完后做 | TBD | 直接影响销售转化 |

### 🟠 P2 · M3 之前的"客户网站深度调研" (D37 加 · per Matthew)
| 项 | 工作量 | 价值 |
|---|---|---|
| **多页 site crawl** · Firecrawl 抓 about/services/各 service / contact 页 · 存 markdown | 4h | redesign 工作量估算 · 真客户文案 / logo / services 进 master.md |
| **客户真 logo 抓取** · 从现网 og:image / favicon / hero img 抽 + Cloudinary upload | 2h | M3 demo 用真 logo 替 reference stock |
| **email / contact_us / social 从 rawHtml 抽** | ✅ DONE (D37 · `core/audit/contact-extraction.js`) | profile card 联系方式 section 实填 |
| **域名年龄 + 注册信息** · paid DomainTools API OR Wayback retry | 3h | 销售开场白用 "您 2008 年注册" 建立信任 |

### 🟠 P2 · Discord display 配套 (D35 follow-up · 让 lifecycle 完整可视化)
| 项 | 工作量 | 价值 |
|---|---|---|
...
| `pl:channels-doctor` (6 channel 健康检查) | 1h | env + tag 同步 + bot 权限 |
| `npm run v3:doc-sync-audit` 实装 | 3h | 强制 SoT 同步契约可执行 |
| #lead-discovery-runs batch thread 接通 | 2h | SOP-1 batch 可视化 |

### ⚪ P4 · 扩展 + future modules
| 项 | 工作量 | 价值 |
|---|---|---|
| Family ≥ 2 (跨 niche reference site · plumber/electrician 等) | 8h | M3 扩到非 roofing niche |
| Photo classification (vision LLM 标 type) | 4h | 真客户图替换 reference stock |
| email / contact_us_url / social_links 数据抓 | 4h | 联系方式 section 填满 (M4 outreach 前置) |
| M4 outreach 启动 (email send + inbound listener + link tracker) | TBD | 💬 👀 emoji 触发 · 整体 funnel 推进 |
| M5 paid lifecycle (Stripe webhook + #paid-websites 自动开 thread) | TBD | 闭环 |
| Hermes cron `rescore-and-audit` 真接通 (jobs.json 加 job) | 30 min | M2 audit 自动跑 (现 manual) |
| 240→11 archive 后批量 grade 回填 (现 grade=null 多) | 30 min · ~$2-4 | 看 funnel 真分布 |
