...
- **2026-05-30 (R93→R110 · copy-quality + layout · COMMITTED + PUSHED · `phase1-audit-detectors`)** · Big session (Claude agent solo for part, then codex-directed review/fixes/commits). Full detail: `docs/v3/HANDOFF-SESSION-2026-05-30-copy-quality.md`. Landed:
  - **Copy bake-off (R93) DONE**: vicwest About rewritten (790w wall → ~230w, real facts). `extract-about.js` = R93 fact-locked contract (drop 800-1200w mandate + projects-estimate fabrication) + `style` safe(default)/flagship + rich factsBlock + count-guard + no-source-boast + demo-placeholder policy. `extract-services.js` / `extract-hero-copy.js` hardened (no $ ranges/fake urgency; H1 5-9w/sub 10-25w). Bake-off finding: the fact-locked CONTRACT fixed copy, not the generation strategy; E (our skill-stack) was net-negative.
  - **Copy gate recalibrated to DEMO-HONESTY policy (R105/106/107)**: `pl-copy-audit` — SOLE honesty hardfail = `fabricated_license_or_identity` (DETERMINISTIC cross-check of rendered licence/ABN/phone/name/address vs `single-page-brief.yaml`); project counts / reviews / AI testimonials / puffery = allowed demo placeholders (advisory); generic/puffery advisory-only; verdict gated only by fabricated-identity + density. `--validate` 3/3 stable (tier_a/identity/density 1.0, acceptable 8/8). gold-set + rubric + optimizer re-encoded.
  - **NEW `core/audit/grid-balance.js`** (wired into `pl-audit-v4` as P1 `grid_balance`) — deterministic "no lone-orphan row in any item-grid" (items%cols==1). Composer (`pl-compose-editorial`) gives reviews/services adaptive columns (4→2×2, KEEP ALL real items, never drop/fabricate) + CSS center-last net for residue. pl-audit-v4 now excludes `index.preview-annotated.html` (audit the publishable page only, index.html first).
  - **NEW `core/audit/persona-copy-judge.js` + `pl:persona-copy-audit`** — buyer-POV copy-QUALITY audit (reads page copy, judges from the client's primary persona `core/audit/personas/*` on 8 criteria → 0-100 persona-fit + concrete gaps; N-run averaged; ADVISORY, not a batch gate). vicwest 71-74/100, would-contact YES. Found + fixed the "File No.·Journal·Vol." magazine-eyebrow residue (→ "City·State·licence").
  - **NEW `core/scoring/module-render-policy.json` (R109 SSOT)** — conditional-module render thresholds + grid policy, read by BOTH composer AND pl-audit-v4 GATE 3. Reconciled the suburbs threshold drift: **3** (audit code always used 3; comments lied "5"; ≥5 would block legit thin a-j). 
  - **Cascade** (`llm-cascade.js`): COPY backups deepseek-r1:14b → **qwen3.6:27b** + `think:false` (design-task backups left on deepseek); `eval_persona_copy` task added.
  - **New tools**: `pl:persona-copy-audit` (buyer-POV quality). Gate split locked: deterministic (honesty/density/voice/structure) = batch gate; subjective quality = advisory + Matthew's eye.
  - Commits (pushed): `f1d49f89` (A render/grid) · `412fca59` (B copy builders + persona audit) · `8e5e4dac` (C gate recalibration) · `8d2dc3b3` (handoff doc) · `8706a292` (R109 policy). **Open: R108 efficiency arch (drafted, not ratified) · enrich-handoff rich-facts · lead pipeline (task 2, awaiting scope) · GitHub secret-scanning flagged a PRE-EXISTING branch secret (not introduced this session — investigate/rotate).**
