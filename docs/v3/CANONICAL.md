...
>
> **STATUS**: v1.5 · locked by 15 codex consensus rounds (R26-R42 + R-BA-6) + Matthew approvals. Lead-capture E2E working · `pl:ship-customer` one-liner · `leads@profitslocal.com` verified sender.
>
...
| **Client form → email** | Every client website MUST have a contact form posting to `/api/client-contact` · `functions/api/client-contact.ts` Resend handler · per-project env `RECIPIENT_EMAIL`+`RESEND_API_KEY`+`FROM_EMAIL` set via `pl:ship-customer` (one-liner: compose→deploy→env→redeploy) · 5 fields (name/email/phone required · service/message optional) · NO Cloudinary · NO hidden tracking (free tier) · SMTP override deferred to paid tier | Codex R42 2026-05-29 + E2E verified | Hidden fields / SMTP = paid-tier upgrade · don't add free |
...
| **Resend sender domain** | `leads@profitslocal.com` — DEFAULT ✅ verified 2026-05-29 · `hello@fengtalk.ai` — legacy fallback (still valid) · Override per project via `FROM_EMAIL` env or `--from` flag | Resend domain dashboard + DNS verified via CF API | Always use `leads@profitslocal.com` for new client deployments |
| **Copy honesty gate (demo-honesty)** | `pl:copy-audit` · SOLE honesty hardfail = `fabricated_license_or_identity` (DETERMINISTIC cross-check of rendered licence/ABN/phone/name/address vs `single-page-brief.yaml`) · density wall deterministic · project counts/reviews/AI-testimonials/puffery = allowed demo placeholders (advisory) · generic/puffery never gates | codex R105/106/107 (2026-05-30) · Matthew demo-honesty directive · `--validate` 3/3 stable | Change the hardfail set / policy = codex round. Identity facts NEVER LLM-judged (deterministic only). |
| **Copy quality (subjective)** | `pl:persona-copy-audit` (`core/audit/persona-copy-judge.js`) · buyer-POV 8-criterion score from `core/audit/personas/*` · ADVISORY + N-run averaged · NOT a batch gate | Matthew 2026-05-30 · vicwest 71-74/100 | Gate split LOCKED: deterministic = batch gate · subjective quality = advisory + Matthew's eye. Do not make persona-fit a hard batch gate. |
| **Grid balance** | No item-grid leaves a LONE item on the last row (`items % cols == 1`). Composer adapts columns to the REAL count (4→2×2), NEVER drops/fabricates items; `core/audit/grid-balance.js` (P1 in pl-audit-v4) flags residue; CSS center-last net | Matthew 2026-05-30 (codex-reviewed) | Layout adapts to content (fluid); complete-set content (services) all shown, sample content (reviews/gallery) may curate. |
| **Render thresholds SSOT** | `core/scoring/module-render-policy.json` — conditional-module render minimums + grid policy, read by BOTH `pl-compose-editorial` AND `pl-audit-v4` GATE 3. suburbs_min = **3** (a-j legit thin client; ≥5 blocks valid output) | codex R109 (2026-05-30) | Change a threshold here only · both sides read this file · no more composer/audit drift. |
| **Copy LLM cascade backup** | COPY tasks (`extract_about/services/hero`, `eval_persona_copy`) local backup = **qwen3.6:27b** + `think:false` (deepseek-r1 retired from copy: ignored the fact-locked contract). DESIGN/fix-matrix tasks stay on deepseek-r1:14b (unverified for qwen) | R93 (2026-05-30) · verified clean JSON | Swapping a backup model = verify it honours the relevant contract first. |
| **Audit page set** | `pl-audit-v4` slug mode audits the PUBLISHABLE page only — EXCLUDES `index.preview-annotated.html` / `preview-old` (derived artifacts that lag live & cause stale findings) · index.html sorted first | codex review 2026-05-30 | Derived previews are never the ship-gate target. |
...
- v1.5 (2026-05-29): profitslocal.com Resend domain verified ✅ (deleted duplicate us-east-1 MX record · all 3 DNS records now passing) · `pl:ship-customer` one-liner added (deploy+env+redeploy in single command) · FROM_EMAIL default → `leads@profitslocal.com` in both client-contact.ts and pl-cf-env-bootstrap.js · `docs/_archive-2026-05-29/` created (22 obsolete docs archived: Dokobot, V2-era plans, old smoke tests) · trade-classic wired as V0 reference family in reference-adapter-handoff.js (replaces retired classic-premium-roftix) · SOP-TEMPLATE-INVENTORY §4.5.e rewritten with 2-command pipeline (compose-editorial + ship-customer).
