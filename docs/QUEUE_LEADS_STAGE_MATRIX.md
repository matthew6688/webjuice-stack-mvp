...
1. 给 `/admin/leads` 增加和 Queue 一致的阶段说明/关键事实区，但保留更完整证据。
2. 把 `build_mockup_artifacts` 里的 placeholder preview/screenshot/video 换成真实 Open Design/template mockup runner 输出。
3. 继续把 selected enrichment 成本关口接到真实执行和入库：当前已支持 dry-run 计划和 `approved` 状态记录，下一步是 `executed / ingested` 自动回写证据。
4. 继续扩展阶段迁移测试：`draft_ready -> outreach_sent -> follow_up_due / replied`。
