...
│  data/leads/queues/cheap-site-audit.json (待审计队列, 列出 keys)  │
...
│  按 cheap-audit action === 'audit_candidate' → detailed-audit   │
└────────────────────────────────────────────────────────────────┘
```
...
**SOP-2 责任**：
- `place_id` 缺失（image-lead 路径）→ 接受，但 cheap-audit niche_match 可能 SKIP
...
| `places_enrichment` | object / null | 可空（G-7 触发后才有）| Places API Details Basic | 增强 audit（types[] / photos / E.164 phone）|
...
| `audit` | detailed-audit 完成 | `{ score, dimension_scores, decision, hard_triggers, issues, audited_at }` |
