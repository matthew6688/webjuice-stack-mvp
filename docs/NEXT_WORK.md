...
- ProfitsLocal branded demo funnel pages are now live on Rich & Rare dev. The template and Rich & Rare repo use a dedicated `FunnelLayout` for `/demo-faq`, `/checkout`, `/contact-us`, `/thank-you`, `/revise`, `/approve`, `/domain-setup`, and `/domain-help`, separate from customer website content. The funnel chrome follows the official ProfitsLocal logo/palette/header/footer language; the preview banner is one line, opens the official site from the logo, includes How it works / FAQ / Contact / Checkout, and hides revision/extra-revision actions until a matched paid order is present.
- `npm run qa:funnel-pages` is implemented as a reusable hard-evidence gate. It now covers 9 routes and checks the official logo link, contact route, checkout source-context fields, no pre-purchase revision actions, utility-page ProfitsLocal chrome, support email, and domain/revision/approval basics.
- The five Brisbane restaurant dev repos are synced to the branded ProfitsLocal funnel chrome and deployed successfully: Longwang `cf6b6a1` / run `25472719823`, Babylon `c7e9cc7` / run `25472721051`, Opa `1982c39` / run `25472721560`, Joey's `d530a63` / run `25472722451`, and Chu The Phat `a53a93c` / run `25472723296`. Each live dev URL passed `qa:funnel-pages` with 59/59 checks.
