...
- `npm run qa:funnel-pages` is implemented as a reusable hard-evidence gate. The first run caught that Rich & Rare's home page lacked the fixed sales footer; commit `9b72b48` restored the footer, GitHub Actions run `25472537209` deployed successfully, and live dev `qa:funnel-pages --base-url https://rich-and-rare-restaurant-dev.pages.dev --client "Rich & Rare Restaurant"` passed 59/59 checks across `/`, `/demo-faq`, `/checkout`, `/thank-you`, `/revise`, `/approve`, `/domain-setup`, and `/domain-help`.
