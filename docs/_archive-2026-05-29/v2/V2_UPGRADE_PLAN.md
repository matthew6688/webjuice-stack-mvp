...
## 已敲定的设计（2026-05-10 更新）

- **Enrichment routing**：本地优先，5 档 fail-soft 降级（Dokobot → DDG → Tinyfish → Firecrawl rotation → Perplexity rotation），完整规范 → [docs/v2/ENRICHMENT_ROUTING.md](ENRICHMENT_ROUTING.md)
- **Multi-key rotation**：Firecrawl 和 Perplexity 都支持多账号；admin 在 `/admin/settings` 维护 key 池元数据（label/cap/spend），key 真值仍在 `.env.local` / Cloudflare secrets。Rotation 选 least-loaded、80% cap stop、429/402 自动 cooldown
- **Ledger 必登记**：所有 provider 调用（含 T0 免费的）都写 ledger，便于成本/收益分析
- **报告样式**：内部 audit = 严肃 monospace dashboard 风（信息密度优先）
- **Proposal page URL**：暂用 `profitslocal.com/p/<slug>`（同域路径，零 DNS 工作）；业务跑稳再切独立子域
- **Phase 1 入口 fixture**：`clients/od-native-clean-roofer-smoke-v2`（已有真实 OD 输出 + `concept-quality-audit.json`）

## 仍未拍板的开放问题

1. **Visual auditor 哪个模型** — Kimi vision / Claude Sonnet vision / GPT-4o vision，由 Phase 1 末段 autoresearch 实测决定，不预设
