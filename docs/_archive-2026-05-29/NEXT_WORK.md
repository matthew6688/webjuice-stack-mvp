...
- ProfitsLocal funnel pages have moved back to the official `profitslocal.com` site. Customer demo repos keep only the one-line preview banner; checkout/contact/FAQ/approval/revision/domain links all point to official pages with preview/repo/client/UTM context.
- `npm run qa:funnel-pages` is implemented as a reusable hard-evidence gate. It now checks the customer homepage banner, official logo/link targets, no pre-purchase revision actions, no local order-status dependency, and verifies old local ProfitsLocal routes are not served by the customer repo.
- The five Brisbane restaurant dev repos were previously synced to the branded ProfitsLocal funnel chrome. The current target is slimmer: customer repos should keep only the preview banner and should send funnel traffic to official `profitslocal.com` pages.
