...
| iter 4 Block E vision autoresearch — 可教学输出（why/correct/fix） | ⏳ | (in progress) | 2 个本地 ollama 模型 × 3 fixtures 跑对比；如本地够用 → T0 零成本 |
| iter 5 Block F internal audit HTML 报告（含 vision + 占位 review/video） | ⏳ |  |  |
| iter 6 review mining (按需，仅高价值 lead) | ⏳ |  |  |
