...
| Sprint F Upsell 机会自动识别 | ✅ | (this commit) | 基于 activity + tech_stack + 评论数据自动识别月度服务包：Social Media Management（dormant/stale + 强口碑） / Social setup + 运营 / 内容写作月度包 / 付费广告 ROI 分析（投广告但没装分析） — 每条带触发依据 + 包内容 + 价格区间 + 销售切入 |
