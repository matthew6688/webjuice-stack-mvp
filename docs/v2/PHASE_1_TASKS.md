...
| iter 5.1 per-issue 硬证据 — cropped 截图 + 慢速 4G 加载视频 | ✅ | `f2542aa` | Brisbane 5/5 issue 都有 on-target 证据；Queensland 站点崩溃自动触发「⚠ 加载失败」证据卡 |
| iter 6 review mining (按需，仅高价值 lead) | ✅ | `63dd1ff` | Google Places 5 reviews/lead × Ollama qwen-nothink 分析 → themes + quotable cards + redesign hooks，3/3 lead 强信号 |
| iter 7 Block G end-to-end pipeline CLI | ✅ | `f5d2e99` | `npm run leads:run-pipeline -- --entity-key X [--with-reviews] [--refetch]`：detailed → vision → reviews → report 一条命令跑通；fixture 智能复用 |
| Sprint 1 上游过滤质量 (niche_mismatch + 第三方平台 + evidence 清理) | ✅ | `9d17b8f` | Hurricane Digital (SEO 代理商) 现在 niche_mismatch → skip；FIX MY ROOF 的 billdu.me → third_party_landing_page → starter_candidate；evidence dir 写入前清空，stale PNG 不再累积 |
| Sprint 2 typography + Cloudinary 资产上传 | ✅ | `0bbe08f` | 中文 h2 + 英文 data 分离 + lead-summary 风格条；evidence/视频上传 Cloudinary，HTML 引用 CDN URL；`--upload-cloudinary` flag |
| Sprint 3 Master MD 合成器 + huashu-md-html 渲染 | ✅ | `1b3951f` | 每个 lead 一份 `master.md`（含 frontmatter + 8 章节中文叙事 + 所有 Cloudinary URL）；`npm run leads:build-master-md -- --html --theme report` 一条命令出 polished HTML；10/10 lead 跑通 |
| Sprint 4 本地 Docker reviews + 技术栈 / pixel / CMS 检测 | ✅ | `ae52a12` | gosom Docker 拉 221 条全量评论（vs Places 5）+ 评分分布；rawHtml 检测 WordPress / GA4 / GTM / Google Ads / Meta Pixel / Hotjar / UA(过期) 等；master.md 加「技术栈与营销基建」+ 评分分布表；Diamond Roof Tiling 自动触发「UA 已 2 年没数据」销售点 |
| Sprint A 三层叙述框架 (技术事实 / 普通话翻译 / 客户影响) | ✅ | (this commit) | 每条 issue 三层渲染：原始事实 + 中文人话 + 商业影响 + 收入语义；DOM 规则用 `core/scoring/rule-narrations.js` 模板（25 条），vision LLM prompt 也强制输出 plain_explanation + customer_impact；vision issues 也进入第五段「漏水」综合渲染 |
| Sprint B SEO 迁移评估 + 运营活跃度 | ✅ | (this commit) | sitemap.xml 解析（页面分类 + redirect plan + 迁移复杂度评级 low/medium/high）+ 活跃度审计（last_modified / blog 数量 / 社交链接 / 整体 freshness 标签）；解决「重做会不会丢 SEO」的客户最大顾虑 |
| Sprint D AI / GEO Readiness 维度 | ✅ | (this commit) | 12 项 AI 时代可发现性检查：llms.txt / robots AI bot 政策 / LocalBusiness/Service/FAQPage/AggregateRating/Breadcrumb JSON-LD / semantic landmarks / FAQ Q&A 格式 / E-E-A-T 凭证（ABN/QBCC/years/insurance）/ warranty；得分 0-100 + 销售切入话术 |
| Sprint E PageSpeed Insights 集成 | ✅ | (this commit) | Google PSI API（25k/day 免费 + 已注册 key 进 admin/settings）拉 mobile + desktop 的真实 Lighthouse 分数 + **CRUX field data**（过去 28 天真实用户体验 p75）+ 优化机会列表（具体节省 ms / KB）；master.md 加「真实速度数据」段 |
| Sprint F Upsell 机会自动识别 | ✅ | `c7f30e3` | 基于 activity + tech_stack + 评论数据自动识别月度服务包：Social Media Management（dormant/stale + 强口碑） / Social setup + 运营 / 内容写作月度包 / 付费广告 ROI 分析（投广告但没装分析） — 每条带触发依据 + 包内容 + 价格区间 + 销售切入 |
| Sprint G 表单审计 + 域名历史 + 邮件 DNS + 业务规模信号 + Pixel 保留清单 | ✅ | (this commit) | form-audit.js（字段数/必填/captcha 类型/honeypot 检测）+ domain-history.js（whois/.au 适配 + Wayback CDX 重试 + SPF/DKIM/DMARC）+ master.md 加「业务规模信号·内部筛选用」段（small/mid/enterprise 分级 + 价格建议）+「Redesign 时必须保留追踪代码」清单（GA4 + Pixel 等 ID 重做必须接回） |
