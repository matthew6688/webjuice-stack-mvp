...
| First-party Stripe checkout | Working MVP | Official `profitslocal.com` owns `/checkout` plus `/api/create-checkout-session`; generated customer artifacts point fixed footer purchase buttons to the official checkout with preview, repo, client, tier, and attribution context. |
| ProfitsLocal preview banner | Working MVP | `webjuice-restaurant` and generated customer repos keep only the fixed preview banner. Local ProfitsLocal utility routes were removed from customer repos; the banner links to official `profitslocal.com` checkout/contact/FAQ/approval/revision/domain pages and hides revision/extra-revision actions until order context is present. `npm run qa:funnel-pages` validates the homepage banner and verifies removed local funnel routes are not served. |
| Stripe paid activation webhook | Working MVP | Official `profitslocal.com` owns Stripe webhook handling; `npm run funnel:route-stripe` normalizes `checkout.session.completed` into revenue ledger and agent task outputs. |
