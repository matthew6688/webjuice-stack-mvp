...
| ProfitsLocal demo funnel chrome | Working MVP | `webjuice-restaurant` and Rich & Rare dev use a dedicated `FunnelLayout` for `/demo-faq`, `/checkout`, `/thank-you`, `/revise`, `/approve`, `/domain-setup`, and `/domain-help`, keeping ProfitsLocal sales/support pages separate from customer website content. Rich & Rare commit `00bf29b` deployed successfully in GitHub Actions run `25470867095`; all utility URLs returned HTTP 200 and static/live checks found no template footer leakage. |
...
2. Add a reusable funnel QA command that checks all customer utility pages for ProfitsLocal branding, pricing links, no template leakage, and HTTP 200 after deploy.
3. Sync the branded funnel pages from `webjuice-restaurant` into the five Brisbane restaurant repos after the Rich & Rare proof stays stable.
4. Harden the admin dashboard with automatic rebuild after actions, operator audit filters, and email drafting/sending.
5. Start the roofing adapter only after the restaurant capsule path stays stable.
